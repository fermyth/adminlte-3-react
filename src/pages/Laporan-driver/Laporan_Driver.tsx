...
function LaporanDriver() {
  ...
  worksheet.mergeCells("A2:M2"); // Diperluas ke kolom M karena ada 2 kolom tambahan
  ...
  worksheet.mergeCells("A3:M3"); // Diperluas ke kolom M
  ...
  // Main headers - ditambahkan 2 kolom baru
  const headers = [
    "No",
    "Nama Driver",
    "Perusahaan",
    ...
    "", // Placeholder for merged cell (foto km_in)
    "Check Out",
    "", // Placeholder for merged cell
    "", // Placeholder for merged cell (foto km_out)
    "Luar Kota",
    "",
  ];
  ...
  worksheet.mergeCells("F4:H4"); // Check In diperluas ke 3 kolom
  worksheet.getCell("F4").value = "Check In";

  worksheet.mergeCells("I4:K4"); // Check Out diperluas ke 3 kolom
  worksheet.getCell("I4").value = "Check Out";

  worksheet.mergeCells("L4:M4"); // Luar Kota tetap 2 kolom
  worksheet.getCell("L4").value = "Luar Kota";
  ...
  subHeaders.forEach((subHeader, index) => {
    worksheet.getCell(`${String.fromCharCode(70 + index)}5`).value = subHeader;
  ...
  // Styling headers - ditambahkan kolom baru
  const headerCells = [
  ...
  if (Array.isArray(data)) {
    data.forEach((item, index) => {
      Object.keys(item.timesheet).forEach((date) => {
        ...
        // Prepare foto URLs
        const fotoKmInUrl = timesheet.km_in_images_cloud || timesheet.km_in_images;
        const fotoKmOutUrl = timesheet.km_out_images_cloud || timesheet.km_out_images;
        ...
        const rowData = [
          ...
          timesheet.km_in !== null && timesheet.km_in !== undefined
            ...
            : "", // KM Masuk
          fotoKmInUrl ? { text: 'Lihat Foto', hyperlink: fotoKmInUrl } : 'Tidak Ada Foto', // Foto KM Masuk
          ...
          timesheet.km_out !== null && timesheet.km_out !== undefined
            ...
            : "", // KM Keluar
          fotoKmOutUrl ? { text: 'Lihat Foto', hyperlink: fotoKmOutUrl } : 'Tidak Ada Foto', // Foto KM Keluar
        ...
        row.eachCell((cell, colNumber) => {
          ...
          if (typeof cell.value === 'object' && cell.value && cell.value.hyperlink) {
            ...
            cell.alignment = {
              ...
              horizontal:
                colNumber === 2 || colNumber === 3 || colNumber === 4
                  ? "left"
                  : "center",
              wrapText: colNumber === 2 || colNumber === 3 || colNumber === 4 || 
                       colNumber === 8 || colNumber === 11 || colNumber === 12 || colNumber === 13,
            };
          }
          ...
          // Apply Rupiah format to "KM Masuk" (column G) and "KM Keluar" (column J)
  ...
  // Set column widths - ditambahkan untuk kolom foto
  const columnWidths = [5, 20, 20, 20, 12, 10, 10, 12, 10, 10, 12, 15, 15];
