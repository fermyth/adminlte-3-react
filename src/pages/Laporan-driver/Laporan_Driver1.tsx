...
const handleDownloadKalim = async () => {
  ...
  // Menggabungkan sel dari A1 hingga H1 untuk judul
  worksheet.mergeCells('A1:H1');
  // Menambahkan judul di sel A1
  worksheet.getCell('A1').value = 'Laporan uang operasional driver';
  // Mengatur judul agar berada di tengah-tengah sel secara vertikal dan horizontal
  worksheet.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' };
  // Membuat judul dengan font tebal dan ukuran font 14
  worksheet.getCell('A1').font = { bold: true, size: 14 };

  // Daftar header untuk kolom
  const headers = [
      'No', 'Nama Driver', 'ID Driver', 'Perusahaan', 'Tanggal', 'Kategory', 'Nilai', 'Keterangan'
  ];
  
  // Menetapkan header pada baris ke-3 dan memberikan style pada setiap header
  headers.forEach((header, index) => {
        const cell = worksheet.getCell(`${String.fromCharCode(65 + index)}3`);
        cell.value = header;
        cell.alignment = { vertical: "middle", horizontal: "center" };
        cell.font = { bold: true, color: { argb: 'FFFFFF' } }; // Font putih untuk header
        cell.fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: '4F81BD' }, // Warna biru gelap untuk header
        };
        cell.border = {
  ...
  data.forEach((item, rowIndex) => {
    // Membuat array untuk setiap baris data yang akan diisi
    const rowData = [
        rowIndex + 1,           // No
  ...
  worksheet.getColumn(2).width = 30;  // Kolom Nama Driver
  worksheet.getColumn(3).width = 10;  // Kolom ID Driver
  worksheet.getColumn(4).width = 30;  // Kolom Perusahaan
  worksheet.getColumn(5).width = 12;  // Kolom Tanggal
  worksheet.getColumn(6).width = 20;  // Kolom Kategory
  worksheet.getColumn(7).width = 15;  // Kolom Nilai
  worksheet.getColumn(8).width = 20;  // Kolom Keterangan
